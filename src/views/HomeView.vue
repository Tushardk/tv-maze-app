  ...
  <main class="mb-24 px-4">
    <div class="md:flex mt-8">
      ...
      <div v-else class="basis-3/4">
        <div v-for="(showGenre, index) in showsByGenres">
          <div v-if="index < 6">
            <router-link :to="{ name: 'allShowsByGenre', params: { genre: getGenre(showGenre) }}">
              <div class="text-md w-fit text-white rounded-full bg-teal-500 px-4 mb-2">{{ getGenre(showGenre) }}</div>
            </router-link>
            <ShowsByGenreList :genre=getGenre(showGenre) />
            <router-link :to="{ name: 'allShowsByGenre', params: { genre: getGenre(showGenre) }}">
              <div class="text-right text-lg text-teal-500 hover:text-teal-800">{{ `See more in ${getGenre(showGenre)}...` }}</div>
            </router-link>
          </div>
        </div>
      </div>
